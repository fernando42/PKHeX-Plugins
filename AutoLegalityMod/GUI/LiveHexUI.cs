...
public partial class LiveHeXUI : Form, ISlotViewer<PictureBox>
{
    ...
    )
    {
        ...
        var valid = IsValid(pkm);
        if (!valid && !_settings.EnableDevMode && InjectionBase.CheckRAMShift(Remote.Bot, out string err))
            return (LiveHeXValidation.RAMShift, err, lv);
        return (LiveHeXValidation.None, "", lv);
    }

    private static bool IsValid(PKM? pkm)
    {
        if (pkm is null)
            return false;
        if (pkm.Species > pkm.MaxSpeciesID || !pkm.ChecksumValid)
            return false;
        if (pkm is { Species: 0, EncryptionConstant: 0 })
            return true;
        if (pkm.Species <= 0)
            return false;
    ...
    private void B_WriteCurrent_Click(object sender, EventArgs e) =>
        Remote.WriteBox(SAV.CurrentBox);

    private void B_ReadSlot_Click(object sender, EventArgs e) =>
        Remote.ReadActiveSlot((int)NUD_Box.Value - 1, (int)NUD_Slot.Value - 1);

    private void B_WriteSlot_Click(object sender, EventArgs e) =>
        Remote.WriteActiveSlot((int)NUD_Box.Value - 1, (int)NUD_Slot.Value - 1);
    ...
    {
        var text = TB_Pointer.Text;
        var ptr = text.Contains("[key]")
            ? text.Replace("[key]", "").Trim()
            : text.Trim();
        return Remote.Bot.GetCachedPointer(sb, ptr, false);
    }
    ...
    private static string GetUrl(LiveHeXValidation validation, ICommunicatorNX nx) => validation switch
    {
